...
public class BookingClient implements AutoCloseable {
    ...
    // 2) <bookingHost> <bookingPort> <user>
        ...
        try (BookingClient client = new BookingClient(bookingHost, bookingPort); Scanner in = new Scanner(System.in)) {
            System.out.println("BookingClient connected (booking=" + bookingHost + ":" + bookingPort + ")");
            if (sessionUser != null) {
                System.out.println("Session user set to '" + sessionUser + "'. You can omit the user in 'book' and 'list' commands.");
            }
            System.out.println("Commands: book <hotel> <yyyy-MM-dd> <durationDays> | pay <bookingId> | cancel <bookingId> | list | ask <city?> <maxDistanceM> <minStars> | quit");
            while (true) {
                System.out.print("> ");
                ...
                try {
                    switch (parts[0]) {
                        ...
                        case "list":
                            ...
                            break;
                        case "pay": {
                            if (parts.length < 2) { System.out.println("Usage: pay <bookingId>"); break; }
                            String user = sessionUser;
                            if (user == null) { System.out.println("Set session user by starting client with username."); break; }
                        ...
                        case "ask": {
                            ...
                            for (Hotel h : resp.getHotelsList()) {
                                ...
                                System.out.print(" price=" + h.getLastPrice() + " (min=" + h.getMinPrice() + ", max=" + h.getMaxPrice() + ")");
